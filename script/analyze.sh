...
cat all-by-pkt.out | grep '^2001:48a8:' | sort | uniq | python count-v6-bits.py | sort -n > merit-observed.pdf
cat merit-observed.pdf | cdf > merit-observed.cdf

#cat merit-observed.pdf | sort -n | uniq -c > merit-observed.counts

cat all-by-pkt.out | sort | uniq | python count-v6-bits.py | sort -n > all-observed.pdf
