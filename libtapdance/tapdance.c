...
// Caller must allocate plaintext to have at least app_data_len bytes.
// *plaintext_len specifies number of bytes that were decrypted
                            ...
                            uint8_t *plaintext, int *plaintext_len)
{
    ...
    *plaintext_len = ssl_decrypt(ssl_obj, app_data, app_data_len, plaintext);
    if (*plaintext_len < 0) {
        SSL_free(ssl_obj);
        return NULL;
    }
