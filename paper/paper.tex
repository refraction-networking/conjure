...
making it difficult for censors to block these addresses outright. 

Using one hour of collected traffic from the same tap we evaluated the entropy seen at
each nybble in a set of IPv6 addresses~\cite{foremski2016entropy}. The entropy and aggregate 
count ratio (ACR) for each position are used to group adjacent nybbles into probabilistically
related segments. We can see that all segments display at least moderate entropy 
suggesting that there is a structure to be mirrored in the way that IPv6 addresses are 
assigned. However, we note that in the dataset that we collected there is no position 
(outside of the upper bits related to the specific /32) that exhibited such low entropy
so as to be a signal to a censor on their own when evaluating a single suspect IPv6 address. 
