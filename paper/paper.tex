...
cat-and-mouse game.
%TODO: don't need to hedge on IPv4 anymore..?
...
%for keeping connections open. We estimate in our deployment an increase of 25\%
%in decoys that could be used.
...
\textbf{application}. The station ignores packets that are not from
...
that this does not address censors that send follow-up probes, as censors could
take over the IP address of suspected clients for these
probes~\cite{ensafi-tor}. However, it does prevent censors from enumerating the
...
two main requirements: first, the protocol it uses to communicate with the
client must be difficult for the censor to passively detect and block by traffic
inspection. Second, the application must resist active probes by the censor (who
...
Tor~\cite{tor} that is designed to address both of these requirements:
it requires each endpoint to have knowledge of a
shared secret (which we can derive from the registration seed),
and attempts to be a protocol with no discernible
...
layer of encryption. This makes it difficult for censors to identify using a
regular expression, as there are no identifying headers or fields for a censor
...
communication. While this can be passively observable by censors (who can also
...
protocol. Nonetheless, this protocol is used successfully in deployed censorship
circumvention systems~\cite{psiphon}. Obfuscated SSH could also be used as a \scheme
...
We implemented the \scheme protocol and deployed it at a 10~Gbps ISP
