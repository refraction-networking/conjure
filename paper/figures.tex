...
\begin{figure}[t]
    ...
    To further investigate this we captured one hour of IPv6 
        traffic at the same network tap and evaluated the entropy and aggregate count ratio 
        (ACR) across each nybble for the set of IPv6 addresses observed~\cite{foremski2016entropy}.
        The low entropy and high ACR in the upper nybbles can be attributed to the commonality 
        of the addresses associated with the specific /32 on which we captured. Overall the
        moderate entropy in most positions suggests that there is a relative structure to the way 
        IPv6 addresses are used but no rigid positions that would betray an individual IPv6 address. 
        Furthermore we can take consideration of that relative entropy when generating phantom 
        addresses.%\looseness=-1
        }
    \label{fig:ipentropy}
\end{figure}
}
...
\begin{figure}[t]
    ...
    \caption{\textbf{IPv6 bits set}---%
        In 16 hours of IPv6 traffic observed on a /32 tap in 
        the ISP's network we counted the number of bits set in each address, and compared this to
        addresses generated from a uniform distribution. If the 96 non-network bits
        ...
        bits set. Nonetheless, the significant overlap of these distributions
